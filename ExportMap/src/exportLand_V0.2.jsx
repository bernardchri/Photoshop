...
var exportLayers = {
    ...
    write : {
        ...
        SCSS : function(){
            ...
            exportSCSS += '\n.map__content{ \n';
            exportSCSS += '\twidth:calc('+  app.activeDocument.width.toString().replace(" px", "px") + ' / 2);\n';
            exportSCSS += '\theight:calc('+  app.activeDocument.height.toString().replace(" px", "px") + ' / 2);\n';
            ...
            for(var i = 0; i < exportLayers.layersVisible.length; i++){
                ...
                exportSCSS += '\t\t@media #{$tablet} {\n';
                // exportSCSS += '\t\t\tposition: absolute;\n';
                exportSCSS += '\t\t\twidth: calc(' +  layerWidth + 'px / 2);\n';
                exportSCSS += '\t\t\theight: calc(' + layerHeight +'px / 2);\n';
                exportSCSS += '\t\t\tleft: calc(' +  posX +' / 2);\n';
                exportSCSS += '\t\t\ttop: calc(' +  posY +' / 2);\n';
                exportSCSS += '\t\t\tbackground: url(\"./assets/img/map-mobile/'+  exportLayers.utils.cleanName( doc.artLayers[i].name ) + ".png" + '\") no-repeat;\n';
        ...
        HTML2 : function(){
            ...
            var backgroundLayers = []; // nommés : &bg_nom-de-calque dans le PSD
            var normalsLayers = []; // nommés : pas de nom specifique
            var buttonLayers = []; // nommés : &button_nom-decalque
            var textLayers = []; // nommés : &button_nom-decalque
            var groupLayers = [];

            //on parse une premiere fois le PSD pour chercher les différents types de layers
            for(var i = exportLayers.layersVisible.length - 1 ; 0 <= i ; i--){
                
    
                var layers = doc.artLayers[i];
                var layerName = doc.artLayers[i].name;

    
                if( layerName.match(/&bg_/)){
                    backgroundLayers.push(layerName.replace(/&bg_/,''));   
                }
  
                else if( layers.kind === LayerKind.TEXT) {
                    textLayers.push( layers );
                }

                else if( layerName.match(/&button_/) ) {
                    buttonLayers.push(layerName);
                }

                else{
                    normalsLayers.push(layerName);
                }
            ...
            //on construits ensuite le html
            //Elements interactifs du type boutton
            exportHTML2 += '<div class="map__buttons">\n';
            for(var buttons = buttonLayers.length - 1 ; 0 <= buttons ; buttons--){
                exportHTML2 += '\t' + buttonLayers[buttons];
                exportHTML2 += '\n';
            }
            exportHTML2 += '</div>\n';
            ...
            exportHTML2 += '<div class="map__txt">\n';
            for(var texts = textLayers.length - 1 ; 0 <= texts ; texts--){
                exportHTML2 += '\t<div class="' + textLayers[texts].name + '" >';
                exportHTML2 += textLayers[texts].textItem.contents;
                exportHTML2 += '</div>\n';
                exportHTML2 += '</div>\n';
            ...
            //Elements géré comme des balises img pour contribuer
            for(var norm = normalsLayers.length - 1 ; 0 <= norm ; norm--){
                exportHTML2 += '\t' + normalsLayers[norm];
                exportHTML2 += '\n';
            }

            //Elements du background gérés dans la css en background
            exportHTML2 += '<div class="map__bgElements">\n';
            
            for(var back = backgroundLayers.length - 1 ; 0 <= back ; back--){
                exportHTML2 += '\t' + '<div class="'+ backgroundLayers[back]+'"></div>';
                exportHTML2 += '\n';
            }
            exportHTML2 += '</div>';
        
            //save
            exportLayers.save(exportHTML2, "/map2.html");
        },
